...
def render_rays(models,
    ...
    def inference(results, model, typ, xyz, z_vals, test_time=False, **kwargs):
        ...
        if typ =='coarse' and test_time and 'fine' in models:
            ...
            edge_sigmas = rearrange(out, '(n1 n2) 1 -> n1 n2', n1=N_rays, n2=N_samples_)
            ...
            out = torch.cat(out_chunks, 0)
            out = rearrange(out, '(n1 n2) c -> n1 n2 c', n1=N_rays, n2=N_samples_, c=2)
            rgbs = out[..., :1]  # (N_rays, N_samples_, 1)
            edge_sigmas = out[..., 1]    # (N_rays, N_samples_)
            results[f'spacial_rgbs_{typ}'] = rgbs.squeeze()

        results[f'edge_sigmas_{typ}'] = edge_sigmas
        sigmas, beta = models['edge2density'](edge_sigmas)
    ...
    results = {}
    beta = inference(results, models['coarse'], 'coarse', xyz_coarse, z_vals, test_time, **kwargs)
    ...
    if N_importance > 0: # sample points for fine model
        ...
        beta = inference(results, models['fine'], 'fine', xyz_fine, z_vals, test_time, **kwargs)
