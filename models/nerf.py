...
class NeRF(nn.Module):
    def __init__(self,
        ...
        # ===========================================================
        for i in range(D2):
            layer = nn.Sequential(nn.Linear(W, W), nn.ReLU(True))
            setattr(self, f"xyz_encoding_final_{i + 1}", layer)
        ...
        self.rgb = nn.Sequential(nn.Linear(W // 2, 1), nn.Sigmoid())
    ...
    def forward(self, x, sigma_only=False):
        ...
        for i in range(self.D2):
            xyz_encoding_final = getattr(self, f"xyz_encoding_final_{i+1}")(xyz_encoding_final)
